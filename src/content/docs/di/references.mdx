...
const generatedNumber = di.derive([], () => Math.random());
const generatedString = di.derive(
  [id, generatedNumber],
  (id, generatedNumber) => id + ' - ' + generatedNumber
);

const printString = di.derive(
  [
    // Will generate different numbers
    di.inject(generatedString, { id: 'a' }),
    di.inject(generatedString, { id: 'b' })
  ],
  (string1, string2) => {
    console.log(string1);
    console.log(string2);
...
Example output:
```
a - 0.3950104748122306
b - 0.4041106096869628
```

It is recommended to **not** have a dependency graph like the above examples. This section is only here for users to be aware of the quirks.
