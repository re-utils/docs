...
Example usage:
<Tabs>
  <TabItem label="src/services.ts" icon="seti:typescript">
    ```ts
  ...
  <TabItem label="src/routes/posts.ts" icon="seti:typescript">
    ```ts
...
<Tabs>
  <TabItem label="src/server.ts" icon="seti:typescript">
    ```ts
    import { derive } from 'udic';
    import { DatabaseSync } from 'node:sqlite';
